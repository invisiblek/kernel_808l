...
static struct mfc_cache *ipmr_cache_find(struct mr_table *mrt,
					 ...
					 __be32 mcastgrp)
{
//Bernie modify for igmpv2 and v3.
#if 0
	int line = MFC_HASH(mcastgrp, origin); //origin kernel code.
	struct mfc_cache *c;
	...
	list_for_each_entry(c, &mrt->mfc_cache_array[line], list) {
		if (c->mfc_origin == origin && c->mfc_mcastgrp == mcastgrp)
		{
			//printk("\n[Bernie Debug]:SSF  c->mfc_origin == origin =%d , return c;\n",c->mfc_origin);
			return c;
		}
	}
	...
	//{
		...
		list_for_each_entry(c, &mrt->mfc_cache_array[line], list) 
		{
			if (c->mfc_origin == origin && c->mfc_mcastgrp == mcastgrp)
			{
				//printk("\n[Bernie Debug]:all source  c->mfc_origin == origin =%d , return c;\n",c->mfc_origin);
				return c;
			}
		}
	//}
...
//Bernie added, pass igmpv3 report msg to user space.
#ifndef CONFIG_IP_MROUTE_MULTIPLE_TABLES
bool query_mrt_mroute_sk(struct net *net)
{
	struct mr_table	*mrt = net->ipv4.mrt;
	if(mrt && mrt->mroute_sk!=NULL)
	{
		return true;
	}
...
EXPORT_SYMBOL(query_mrt_mroute_sk);
#endif