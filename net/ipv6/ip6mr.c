...
{
	//int line = MFC6_HASH(mcastgrp, origin); Bernie modify
	int line = MFC6_HASH(mcastgrp, &in6addr_any);
	struct mfc6_cache *c;

	list_for_each_entry(c, &mrt->mfc6_cache_array[line], list) {
		if ((ipv6_addr_equal(&c->mf6c_origin, &in6addr_any) || ipv6_addr_equal(&c->mf6c_origin, origin)) &&
			 ...
			 struct mf6cctl *mfc, int mrtsock)
{
	...
	list_for_each_entry(c, &mrt->mfc6_cache_array[line], list) {
		//if (ipv6_addr_equal(&c->mf6c_origin, &mfc->mf6cc_origin.sin6_addr) &&
		if((ipv6_addr_equal(&c->mf6c_origin, &in6addr_any) || ipv6_addr_equal(&c->mf6c_origin, &mfc->mf6cc_origin.sin6_addr)) &&
		    ipv6_addr_equal(&c->mf6c_mcastgrp, &mfc->mf6cc_mcastgrp.sin6_addr)) {
			found = true;
			break;
		}
	...
	ipv6_addr_copy(&c->mf6c_origin, &mfc->mf6cc_origin.sin6_addr);
	ipv6_addr_copy(&c->mf6c_mcastgrp, &mfc->mf6cc_mcastgrp.sin6_addr);
	c->mf6c_parent = mfc->mf6cc_parent;
	ip6mr_update_thresholds(mrt, c, ttls);
	if (!mrtsock)
		c->mfc_flags |= MFC_STATIC;
