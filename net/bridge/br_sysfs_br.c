...
static ssize_t show_guest_zone(struct device *d, struct device_attribute *attr,
			      char *buf)
{
	struct net_bridge *br = to_bridge(d);
	return sprintf(buf, "%d\n", br->guestzone_enabled);
}

static int set_guest_zone(struct net_bridge *br, unsigned long val)
{
	if(val)
		br->guestzone_enabled = 1;
	else
		br->guestzone_enabled = 0;
	return 0;
}

static ssize_t store_guest_zone(struct device *d, struct device_attribute *attr,
			       const char *buf, size_t len)
{
	return store_bridge_parm(d, buf, len, set_guest_zone);
}
static DEVICE_ATTR(guest_zone, S_IRUGO | S_IWUSR, show_guest_zone, store_guest_zone);

static ssize_t show_bridge_ip(struct device *d, struct device_attribute *attr,
			      char *buf)
{
	struct net_bridge *br = to_bridge(d);
	struct in_device *in_dev = __in_dev_get_rtnl(br->dev);
	return sprintf(buf, "%x\n", ntohl(in_dev->ifa_list->ifa_local));
}
static DEVICE_ATTR(bridge_ip, S_IRUGO, show_bridge_ip, NULL);

static ssize_t show_tcp_allow_list(struct device *d, struct device_attribute *attr,
			      char *buf)
{
	struct net_bridge *br = to_bridge(d);
	int i = 0;
...
static ssize_t store_tcp_allow_list(struct device *d, struct device_attribute *attr,
			       const char *buf, size_t len)
{
	struct net_bridge *br = to_bridge(d);
	char *ptr = (char *)buf;
	...
	else
	{
		do {
			...
			{
				ptr++;
				port = simple_strtol(ptr, &end, 10);
				if(ptr != end)
					cameo_br_list_del_one(br, (u16)port, 6);
				else
					break;
			}
			else if(*ptr == '+')
			{
				...
				port = simple_strtol(ptr, &end, 10);
				if(ptr != end)
					cameo_br_list_add(br, (u16)port, 6);
				else
					break;
			}
			else
...
static DEVICE_ATTR(tcp_allow, S_IRUGO | S_IWUSR, show_tcp_allow_list, store_tcp_allow_list);

static ssize_t show_udp_allow_list(struct device *d, struct device_attribute *attr,
			      char *buf)
{
	struct net_bridge *br = to_bridge(d);
	int i = 0;
...
static ssize_t store_udp_allow_list(struct device *d, struct device_attribute *attr,
			       const char *buf, size_t len)
{
	struct net_bridge *br = to_bridge(d);
	char *ptr = (char *)buf;
	...
	else
	{
		do {
			if(*ptr == '-')
			{
				...
				port = simple_strtol(ptr, &end, 10);
				if(ptr != end)
					cameo_br_list_del_one(br, (u16)port, 17);
				else
					break;
			}
			else if(*ptr == '+')
			{
				...
				port = simple_strtol(ptr, &end, 10);
				if(ptr != end)
					cameo_br_list_add(br, (u16)port, 17);
				else
					break;
			}
			else
...
static DEVICE_ATTR(udp_allow, S_IRUGO | S_IWUSR, show_udp_allow_list, store_udp_allow_list);
#endif
/* Dino */
...
/* Dino @20120413 */
#ifdef CONFIG_WIRELESS_GUEST_ZONE
	&dev_attr_guest_zone.attr,
	&dev_attr_bridge_ip.attr,
	&dev_attr_tcp_allow.attr,
	&dev_attr_udp_allow.attr,
#endif
/* Dino */
	NULL
};
