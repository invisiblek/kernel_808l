 ...
 */
static u8 tcp_conntracks[2][6][TCP_CONNTRACK_MAX] = {
	{
/* ORIGINAL */
...
void spi_syncack_check(int enable)
{
	if (enable)
	{
		tcp_conntracks[IP_CT_DIR_REPLY][TCP_SYNACK_SET][TCP_CONNTRACK_ESTABLISHED] = sIV;
	}
	else
	{
		tcp_conntracks[IP_CT_DIR_REPLY][TCP_SYNACK_SET][TCP_CONNTRACK_ESTABLISHED] = sIG;
	}

	return;
}
EXPORT_SYMBOL_GPL(spi_syncack_check);
			  ...
			  u_int8_t pf)
{
	...
	if (before(seq, sender->td_maxend + 1) &&
		...
		//if (!res && LOG_INVALID(net, IPPROTO_TCP))  //show spi log 
		      ...
		      unsigned int hooknum)
{
	...
	if (spi_enable && !tcp_in_window(ct, &ct->proto.tcp, dir, index,
			   skb, dataoff, th, pf)) {
		spin_unlock_bh(&ct->lock);
		return -NF_ACCEPT;
	}
...
/* Dino @20120203 */
	...
	cameo_nf_ct_refresh_acct_tcp(ct, ctinfo, skb, timeout, (void *)old_state, (void *)new_state);
#else
/* Dino */
