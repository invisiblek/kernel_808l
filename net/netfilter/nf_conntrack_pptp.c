...
static int nf_conntrack_pptp_cameo_init(void);
static void nf_conntrack_pptp_cameo_fini(void);

static struct proc_dir_entry *proc_pptp = NULL;
static int pptp_enable = 1;

static int pptp_read_proc(char *page, char **start, off_t off,
...
static int pptp_write_proc(struct file *file, const char *buffer,
...
{      
	...
	{
		sscanf(tmpbuf, "%d", &newValue);	

		if (newValue!=pptp_enable)
		{
			pptp_enable = newValue;

			if(pptp_enable) {
				nf_nat_proto_gre_cameo_init();
				nf_nat_pptp_cameo_init();
				nf_ct_proto_gre_cameo_init();
				nf_conntrack_pptp_cameo_init();
			} else {
				nf_nat_proto_gre_cameo_fini();
				nf_nat_pptp_cameo_fini();
				nf_conntrack_pptp_cameo_fini();
				nf_ct_proto_gre_cameo_fini();
			}
		}
...
static int __init nf_conntrack_pptp_init(void)
{
	...
	proc_pptp=create_proc_entry("pptp",0,NULL);
	if (proc_pptp)
	{
	    proc_pptp->read_proc=pptp_read_proc;
	    proc_pptp->write_proc=pptp_write_proc;
	}
	return rv;
}
