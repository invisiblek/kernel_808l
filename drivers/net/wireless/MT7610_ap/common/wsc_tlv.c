	...
	OUT	VOID *pbuf)
{
	...
	if (pWscControl->WscV2Info.bEnableWpsV2)
	{
		...
		*/
		...
		{
			//ConfigMethods = (pWscControl->WscConfigMethods & 0x210F);//for pass WPS Logo wire ER.
			ConfigMethods = (pWscControl->WscConfigMethods & 0x20FF);//for dlink spec
		}
	}
		else
#endif /* WSC_V2_SUPPORT */
...
/*	IV_EncrData = kmalloc(512, MEM_ALLOC_FLAG); */
		...
		(pWscControl->EntryIfIdx < MIN_NET_DEVICE_FOR_APCLI))
    {
		...
		*/
		...
		{
			if(pWscControl->bSetupLock == FALSE)
			{
				DBGPRINT(RT_DEBUG_ERROR, ("2012.04.12. Win7 Detect WPS 1.0 Devcie change to WPA-PSK/WPA2-PSK Mix mode!\n"));
