	...
	IN PRTMP_ADAPTER pAd)
{
	...
	/* DLINK CCP SUPPORT
	?™è£¡?‰ä??‹patch?¯D-Link request, ?ºä?è§?±ºhidden node?„å?é¡?	?¶å…©?°ä»¥ä¸ŠSTA????? AP beacon HT info?ƒå¸¶mix mode, å¼·åˆ¶è«‹STA?¼RTS
	?™æ˜¯for Cameo only?„patch, è«‹æ³¨?? ?™å€‹patch?ƒè?Wifi logo test 4.2.26 FAIL
	?æ¸¬WiFi logo?‚è??œæ?
	*/
	if (pAd->MacTab.fAnyStationIsLegacy
#ifdef EAP_CCP_SUPPORT
	|| (pAd->MacTab.Size>1)	
#endif
	)
	{
...

{
	...
	)
	{
		...
		/* 2012/10/11 MTK patch ?¶AP fall back to 20M?„æ??? base band registeræ²’æ?fall back to 20M*/
