...
static void rtmp_read_radius_parms_from_file(IN  PRTMP_ADAPTER pAd, PSTRING tmpbuf, PSTRING buffer)
{
	...
	offset = 0;
	/*if (RTMPGetKeyParameter("RADIUS_Server", tmpbuf, 256, buffer, TRUE))*/
	while (RTMPGetKeyParameterWithOffset("RADIUS_Acct_Server", tmpbuf, &offset, 256, buffer, TRUE))	
	{
		for (i=0, macptr = rstrtok(tmpbuf,";"); (macptr && i < MAX_MBSSID_NUM(pAd)); macptr = rstrtok(NULL,";"), i++) 
		{
			if (rtinet_aton(macptr, &ip_addr) && pAd->ApCfg.MBSSID[i].radius_srv_num < MAX_RADIUS_SRV_NUM)
			{
				...
				pAd->ApCfg.MBSSID[i].radius_srv_info[srv_idx].radius_ip = ip_addr;
				pAd->ApCfg.MBSSID[i].radius_srv_num++;
				DBGPRINT(RT_DEBUG_TRACE, ("IF(ra%d), radius_ip(seq-%d)=%s(%x)\n", i, srv_idx, macptr, pAd->ApCfg.MBSSID[i].radius_srv_info[srv_idx].radius_ip));
			}	    
		}
	}
	/* RADIUS_Port*/
	...
	offset = 0;
	memset(&count[0], 0, sizeof(count));
	while (RTMPGetKeyParameterWithOffset("RADIUS_Acct_Port", tmpbuf, &offset, 128, buffer, TRUE))
	{
		for (i=0, macptr = rstrtok(tmpbuf,";"); (macptr && i < MAX_MBSSID_NUM(pAd)); macptr = rstrtok(NULL,";"), i++) 
		{	  
			if (count[i] < pAd->ApCfg.MBSSID[i].radius_srv_num)
			{		
            	...
            	pAd->ApCfg.MBSSID[i].radius_srv_info[srv_idx].radius_port = (UINT32) simple_strtol(macptr, 0, 10); 
				count[i] ++;
				DBGPRINT(RT_DEBUG_TRACE, ("IF(ra%d), radius_port(seq-%d)=%d\n", i, srv_idx, pAd->ApCfg.MBSSID[i].radius_srv_info[srv_idx].radius_port));
			}
		}
	}
	/* RADIUS_Key*/
	...
	offset = 0;
	memset(&count[0], 0, sizeof(count));
	while (RTMPGetKeyParameterWithOffset("RADIUS_Acct_Key", tmpbuf, &offset, 640, buffer, FALSE))
	{
		if (strlen(tmpbuf) > 0)
			bUsePrevFormat = TRUE;
	...
	/*
	DBGPRINT(RT_DEBUG_TRACE, ("ID0 key0= %s\n",pAd->ApCfg.MBSSID[0].radius_srv_info[0].radius_key));
	DBGPRINT(RT_DEBUG_TRACE, ("ID0 key0= %s\n",pAd->ApCfg.MBSSID[0].radius_srv_info[0].radius_key));

	DBGPRINT(RT_DEBUG_TRACE, ("ID0 key1= %s\n",pAd->ApCfg.MBSSID[0].radius_srv_info[1].radius_key));
	DBGPRINT(RT_DEBUG_TRACE, ("ID0 key1= %s\n",pAd->ApCfg.MBSSID[0].radius_srv_info[1].radius_key));

	DBGPRINT(RT_DEBUG_TRACE, ("ID1 key0= %s\n",pAd->ApCfg.MBSSID[1].radius_srv_info[0].radius_key));
	DBGPRINT(RT_DEBUG_TRACE, ("ID1 key0= %s\n",pAd->ApCfg.MBSSID[1].radius_srv_info[0].radius_key));

	DBGPRINT(RT_DEBUG_TRACE, ("ID1 key1= %s\n",pAd->ApCfg.MBSSID[1].radius_srv_info[1].radius_key));
	DBGPRINT(RT_DEBUG_TRACE, ("ID1 key1= %s\n",pAd->ApCfg.MBSSID[1].radius_srv_info[1].radius_key));
	*/
	...
	IN RTMP_ADAPTER *pAd)
{
	...
	DBGPRINT(RT_DEBUG_ERROR,
		("-->Hello Cameo Opening MT7620 SingleSKU %s success\n", SINGLE_SKU_TABLE_FILE_NAME));
	...
	IN INT32 paValue)
{
	...
	pwr_diff = pwr_diff >> 12;
       ...
       DBGPRINT(RT_DEBUG_TRACE, ("%s: pwr_diff = 0x%x, rate_pwr = 0x%x, base_pwr = 0x%x !!!\n",
