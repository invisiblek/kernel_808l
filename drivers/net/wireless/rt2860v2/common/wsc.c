...
#endif /* IWSC_SUPPORT */
		/* 
			Patch for Win 8 Logo test , 
			sometimes selected registrar config method will become 0x0000.
			it will cause win 8 fail 
		*/
		/*
		printk("selRegCfgMethods=%04x",selRegCfgMethods);
		if(selRegCfgMethods==0x0)
			selRegCfgMethods=0x200c;
		*/
...
#ifdef WSC_V2_SUPPORT
		if (pWpsCtrl->WscV2Info.bEnableWpsV2){
			//DBGPRINT(RT_DEBUG_ERROR, ("%s: pWpsCtrl->WscConfigMethods=0x%04x!\n", __FUNCTION__,pWpsCtrl->WscConfigMethods));
			tempVal = pWpsCtrl->WscConfigMethods & 0xF8FF;
		}
		else
#endif /* WSC_V2_SUPPORT */
...
/* 2012 0418 ,Fix Intel proset issue for MTK Justin*/
#if 0
	...
	*/
		...
		)
	{
		...
		for (ap_idx = 0; ap_idx < MaxBssidNum; ap_idx++)
		{
			...
			/* 2012 0418 ,Fix Intel proset issue for MTK Justin*/
			if(pWpsCtrl->bM2DTimerRunning)
			{
				BOOLEAN Cancelled;
				pWpsCtrl->bM2DTimerRunning = FALSE;
				RTMPCancelTimer(&pWpsCtrl->M2DTimer, &Cancelled);
			}
	...
	IN PVOID SystemSpecific3)
{
	...
	if (WscGetAuthMode(pCredential->AuthType) == pMbss->AuthMode &&
		...
		pMbss->SsidLen == pCredential->SSID.SsidLength
		)
	{
		return;
	}
