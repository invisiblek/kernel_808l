...
/* cameo brian add */
char* GetPhyMode_dlink(
	int Mode,UCHAR PhyMode)
{
	switch(Mode)
	{
		case MODE_CCK:
		...
		case MODE_OFDM:
			if(PHY_MODE_IS_5G_BAND(PhyMode)==1)
				return "802.11a";
			else
				return "802.11g";
#ifdef DOT11_N_SUPPORT
...
int GetSignal_dlink(
	CHAR RSSIA,CHAR RSSIB)
{
#ifdef CAMEO_NETWORK_DIR808
	int avg = abs((int)RSSIA);
#else
	int avg = abs(((int)RSSIA + (int)RSSIB)/2);
#endif
	if( avg <= 50 )
		return 100;
	else
	{
		return (int)(100-((avg-50)*2));
	}
}
