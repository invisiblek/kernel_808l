	...
	IN PRTMP_ADAPTER pAd)
{
	...
	/* DLINK CCP SUPPORT
        這裡有一個patch是D-Link request, 為了解決hidden node的問題
        當兩台以上STA連上時, AP beacon HT info會帶mix mode, 強制請STA發RTS
        這是for Cameo only的patch, 請注意, 這個patch會讓Wifi logo test 4.2.26 FAIL
        送測WiFi logo時請關掉
