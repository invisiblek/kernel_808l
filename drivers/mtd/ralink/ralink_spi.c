...
//CAMEO_ADD
				...
				name:			"5G_RF",
				size:			MTD_5G_RF_PART_SIZE,
				offset: 		MTDPART_OFS_APPEND
		}, {
				name:			"Wolf_Config",
				size:			MTD_WOLF_CONFIG_PART_SIZE,
				offset: 		MTDPART_OFS_APPEND
		}, {
				name:			"MyDLink",
				size:			MTD_MYDLINK_PART_SIZE,
				offset: 		MTDPART_OFS_APPEND
		}, {
				name:			"Jffs2",
...
//CAMEO_MODIFY
				...
				size:           MTD_KERNEL_ROOTFS_PART_SIZE,
				offset: 		MTD_BOOT_PART_SIZE + MTD_CONFIG_PART_SIZE + MTD_FACTORY_PART_SIZE + MTD_5G_RF_PART_SIZE + MTD_WOLF_CONFIG_PART_SIZE + MTD_MYDLINK_PART_SIZE + MTD_JFFS2_PART_SIZE,
//CAMEO_END
#endif
...
#endif
{
	...
	offs = MTD_BOOT_PART_SIZE + MTD_CONFIG_PART_SIZE + MTD_FACTORY_PART_SIZE + MTD_5G_RF_PART_SIZE + MTD_WOLF_CONFIG_PART_SIZE + MTD_MYDLINK_PART_SIZE + MTD_JFFS2_PART_SIZE;
	ramtd_read(NULL, offs, sizeof(hdr), (size_t *)&i, (u_char *)(&hdr));
	if (hdr.ih_ksz != 0) {
		rt2880_partitions[8].size = ntohl(hdr.ih_ksz);
		//rt2880_partitions[9].size = MTD_KERNEL_ROOTFS_PART_SIZE - ntohl(hdr.ih_ksz);
		rt2880_partitions[9].size = IMAGE1_SIZE - (MTD_BOOT_PART_SIZE + MTD_CONFIG_PART_SIZE + MTD_FACTORY_PART_SIZE + MTD_5G_RF_PART_SIZE + 
									MTD_WOLF_CONFIG_PART_SIZE + MTD_MYDLINK_PART_SIZE + MTD_JFFS2_PART_SIZE + ntohl(hdr.ih_ksz));
	}
#endif
